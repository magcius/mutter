...
meta_window_actor_damage_all (MetaWindowActor *self)
{
  ...
  meta_shaped_texture_get_size (META_SHAPED_TEXTURE (priv->actor),
                                &width, &height);
  ...
  if (priv->back_pixmap == None)
    {
      ...
      tex = meta_shaped_texture_set_pixmap (META_SHAPED_TEXTURE (priv->actor),
                                            priv->back_pixmap);
       ...
       */
      if (G_UNLIKELY (!cogl_texture_pixmap_x11_is_using_tfp_extension (tex)))
