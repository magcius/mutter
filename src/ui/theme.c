...
static void
meta_frame_layout_calc_geometry (GtkStyleContext        *ctx,
                                 MetaFrameFlags          flags,
                                 MetaFrameType           type,
                                 int                     client_width,
                                 int                     client_height,
                                 ...
                                 MetaTheme              *theme)
{
  meta_frame_layout_get_borders (ctx, flags, type, &fgeom->borders);
  fgeom->width = client_width + fgeom->borders.total.left + fgeom->borders.total.right;
  fgeom->height = ((flags & META_FRAME_SHADED) ? 0: client_height) +
    fgeom->borders.total.top + fgeom->borders.total.bottom;
}
